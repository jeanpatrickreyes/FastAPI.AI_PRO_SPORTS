-- AI PRO SPORTS - Database Initialization Script
-- Version: 2.0.0
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'ai_pro_sports_app') THEN
        CREATE USER ai_pro_sports_app WITH PASSWORD 'app_password_change_me';
    END IF;
END
$$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'ai_pro_sports_readonly') THEN
        CREATE USER ai_pro_sports_readonly WITH PASSWORD 'readonly_password_change_me';
    END IF;
END
$$;
...
GRANT CONNECT ON DATABASE ai_pro_sports TO ai_pro_sports_app;
GRANT USAGE ON SCHEMA public TO ai_pro_sports_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO ai_pro_sports_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO ai_pro_sports_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ai_pro_sports_app;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO ai_pro_sports_app;

-- Read-only user permissions
GRANT CONNECT ON DATABASE ai_pro_sports TO ai_pro_sports_readonly;
GRANT USAGE ON SCHEMA public TO ai_pro_sports_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO ai_pro_sports_readonly;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO ai_pro_sports_readonly;
...
ALTER DATABASE ai_pro_sports SET statement_timeout = '300s';

-- Set work_mem for complex queries
ALTER DATABASE ai_pro_sports SET work_mem = '256MB';

-- Set maintenance_work_mem for maintenance operations
ALTER DATABASE ai_pro_sports SET maintenance_work_mem = '512MB';

-- Enable JIT compilation
ALTER DATABASE ai_pro_sports SET jit = on;
...
DO $$
BEGIN
    RAISE NOTICE 'AI PRO SPORTS database initialized successfully at %', CURRENT_TIMESTAMP;
END
$$;
