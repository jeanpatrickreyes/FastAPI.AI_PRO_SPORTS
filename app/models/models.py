...
class Prediction(Base):
    ...
    signal_tier: Mapped[SignalTier] = mapped_column(Enum(SignalTier), default=SignalTier.D)
...
class PredictionResult(Base):
    ...
    actual_result: Mapped[BetResult] = mapped_column(Enum(BetResult), nullable=False)
    closing_line: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
...
class PlayerProp(Base):
    ...
    result: Mapped[Optional[BetResult]] = mapped_column(Enum(BetResult), nullable=True)
...
class MLModel(Base):
    ...
    framework: Mapped[MLFramework] = mapped_column(Enum(MLFramework), nullable=False)
...
class Bet(Base):
    ...
    result: Mapped[BetResult] = mapped_column(Enum(BetResult), default=BetResult.PENDING)
...
class ScheduledTask(Base):
    ...
    status: Mapped[TaskStatus] = mapped_column(Enum(TaskStatus), default=TaskStatus.IDLE)
...
class SystemHealthSnapshot(Base):
    ...
    status: Mapped[HealthStatus] = mapped_column(Enum(HealthStatus), default=HealthStatus.HEALTHY)
